...
const create = async (req, res) => {
  try {
    ...
    if (!location || !model || !serialNumber) {
      res.status(400).send({
        message: "Submit all fields to register hydrometer"
      });
    }
    await (0, _hydrometerService.createService)({
      location,
      model,
      serialNumber,
      user: {
        _id: "65468e202145bd27a81b006f"
      }
    });
    res.status(201).send({
...
const findAll = async (req, res) => {
  try {
    const hydrometers = await (0, _hydrometerService.findAllService)();
...
const update = async (req, res) => {
  ...
  try {
    ...
    const updatedHydrometer = await (0, _hydrometerService.updateService)(id, updateData);
    res.send({
      message: "Hydrometer successfully updated",
      updatedHydrometer
    });
  } catch (err) {
    res.status(500).send({
...
const valveStatus = async (req, res) => {
  try {
    ...
    res.send({
      valveState: hydrometer.valveState
    });
  } catch (err) {
    res.status(500).send({
      message: err.message
